...
use PHPUnit\Runner\Exception as PHPUnitRuntimeException;
...
include "Dependencies/vendor/autoload.php";
    ...
    public function run(): void
    {
        ...
        try {
            ...
            try {
                $result = $phpunit->run(
                    $phpunit->getTest(
                        $this->solutionPath . 'tests' . DIRECTORY_SEPARATOR,
                        ''
                    ),
                    ['extensions' => []]
                );
                echo $result->getConvertErrorsToExceptions();
            } catch (PHPUnitRuntimeException $e) {
                print $e->getMessage() . "\n";
...
$kataboardRunner->findWorkplaceSolutionByName($consoleOptions['s'] ?? 'default');
$kataboardRunner->run();
